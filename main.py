...
def extract_exe_metadata(exe_path):
    try:
        ...
        metadata = {
            ...
            "TimeDateStamp": pe.FILE_HEADER.dump_dict()["TimeDateStamp"]["Value"].split(
                "["
            )[1][:-1],
            ...
            "Exports": [
                entry.name.decode() for entry in pe.DIRECTORY_ENTRY_EXPORT.symbols
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=f"Metalookup Version {Version}"
    )
    ...
    group.add_argument(
        "-f",
        "--file",
        help="File to extract metadata from or detect its type",
        type=str,
    )
    group.add_argument(
        "-d",
        "--directory",
        help="Directory to extract metadata from all contained files",
        type=str,
    )
    ...
    parser.add_argument(
        ...
        "--detect",
        action="store_true",
        help="Detect the file type based on its magic number. Requires -f.",
    )
    ...
    if args.detect and args.directory:
        print(
            "Error: The -D (detect) option cannot be used together with the -d (directory) option."
        )
    elif args.file:
